...
def put_player(player_id, personal_name, discord_id):
    """添加玩家信息"""
    connection = get_mysql_connection()
    if connection:
        ...
        try:
            ...
            cursor.execute(
                "INSERT INTO players (player_id, personal_name, discord_id) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE personal_name = %s, discord_id = %s",
                (player_id, personal_name, discord_id, personal_name, discord_id)
            )
            connection.commit()
            return True
        except Exception as e:
